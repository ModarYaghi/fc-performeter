...
def merge_strategy(column_values: pd.Series, column_name: str, strategies: Dict[str, Any]) -> Any:
    ...
    if column_name in strategies:
        ...
        logger.warning(f"No specific merge strategy for column {column_name}. Using first non-null value.")
        return next((v for v in column_values if pd.notna(v)), np.nan)

def rebuild_dataframe(df: pd.DataFrame,
                      id_column: str,
    ...
    for chunk in id_chunks:
        ...
        merged_records = grouped.apply(lambda x: pd.Series({
            col: merge_strategy(x[col], col, merge_strategies) for col in df.columns
        }))

        # Append to the result DataFrame
        empty_df = pd.concat([empty_df, merged_records.reset_index(drop=True)], ignore_index=True)
    ...
    if len(empty_df) != len(unique_ids):
        logger.warning(f"Mismatch in record count. Expected {len(unique_ids)}, got {len(empty_df)}")
